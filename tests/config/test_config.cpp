...
TEST(TestConfig, test_merge_empty_source)
{
    std::shared_ptr<Config> config = std::make_shared<Config>();
    std::shared_ptr<Config> source = std::make_shared<Config>();
    std::string expected_backup_strategy = "keep-nth";
    ...
    config->action = expected_action;
    config->backup.strategy = expected_backup_strategy;
    config->backup.nth = expected_backup_nth;
    config->backup.config_file = expected_backup_config_file;
    config->backup.destination = expected_backup_destination;
    config->backup.backup_dir_name = expected_backup_dir_name;
    config->exclusion_paths.insert(expected_path_str);
    config->exclusion_patterns = expected_regex;

    config->include_directories.insert("/Hello");
    config->include_directories.insert("/World");
